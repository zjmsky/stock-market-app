...
namespace StockMarket.Exchange.Api.Services
{
    public class ExchangeRepo
    {
        ...
        {
            var message = new ExchangeIntegrationEvent(EventType.Update, exchange);
            await _events.PublishExchangeEvent(message);
            return true;
        }

        private async Task<bool> PublishDeletion(ExchangeEntity exchange)
        {
            var message = new ExchangeIntegrationEvent(EventType.Delete, exchange);
            await _events.PublishExchangeEvent(message);
            return true;
        }
        ...
        {
            return await _context.Exchanges.EstimatedDocumentCountAsync();
        }
