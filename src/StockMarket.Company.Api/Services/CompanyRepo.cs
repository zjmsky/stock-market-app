...
namespace StockMarket.Company.Api.Services
{
    public class CompanyRepo
    {
        ...
        {
            var message = new CompanyIntegrationEvent(EventType.Update, company);
            await _events.PublishCompanyEvent(message);
            return true;
        }

        private async Task<bool> PublishDeletion(CompanyEntity company)
        {
            var deletionEvent = new CompanyIntegrationEvent(EventType.Delete, company);
            await _events.PublishCompanyEvent(deletionEvent);
            return true;
        }
